...
class OdomComparisonPlotter(Node):
    def __init__(self):
        ...
        self.get_logger().info('Odometry Comparison Plotter started')
    ...
    def setup_csv_logging(self):
        ...
        self.csv_file = f"odom_comparison_{timestamp}.csv"
    ...
    def setup_plot(self):
        ...
        self.ax.set_title('Odometry Comparison: /wc_control/odom vs /odometry/filtered', fontsize=14, fontweight='bold')
        self.ax.grid(True, alpha=0.3)
        self.ax.set_aspect('equal')
        
        # Reference rectangle (10m x 5m) to compare trajectories against
        self.rectangle_width = 10.0
        self.rectangle_height = 5.0
        self.rectangle_patch = patches.Rectangle(
            (0.0, 0.0),
            ...
            facecolor='none',
            linestyle='--',
            label='Reference Rectangle (10m x 5m)'
        )
        self.ax.add_patch(self.rectangle_patch)
        self.ax.set_xlim(-1.0, self.rectangle_width + 1.0)
        self.ax.set_ylim(-1.0, self.rectangle_height + 1.0)
    ...
    def update_plot(self, frame):
        ...
        try:
            with self.data_lock:
                ...
                # Keep axes focused around the reference rectangle with margin
                margin = 1.0
                min_x = min([0.0, self.raw_odom_current['x'], self.filtered_odom_current['x']]) - margin
                max_x = max([self.rectangle_width, self.raw_odom_current['x'], self.filtered_odom_current['x']]) + margin
                min_y = min([0.0, self.raw_odom_current['y'], self.filtered_odom_current['y']]) - margin
                max_y = max([self.rectangle_height, self.raw_odom_current['y'], self.filtered_odom_current['y']]) + margin
...
def main(args=None):
    ...
    try:
        ...
        print("Plot will show real-time odometry comparison")
